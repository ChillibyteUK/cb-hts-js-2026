 ...
 * @return void
 */
function cb_hts_js_2026_register_blocks() {
	foreach ( glob( get_template_directory() . '/blocks/*/block.json' ) as $block_json ) {
		register_block_type( dirname( $block_json ) );
	}
}
add_action( 'init', 'cb_hts_js_2026_register_blocks' );

/**
 * Enqueue the shared horizontal-marquee ticker script, only on pages using a
 * block that renders a [data-marquee] element (CB Marquee Stats, CB Selected
 * Clients). Registered by hand (rather than via block.json's "viewScript",
 * which auto-registers with no dependencies) so it can declare "gsap" as a
 * dependency and print after it. Lives in blocks/_shared/ rather than either
 * block's own directory since both use it identically.
 *
 * @return void
 */
function cb_hts_js_2026_enqueue_marquee_view_script() {
	$marquee_blocks = array( 'cb-hts-js-2026/cb-marquee-stats', 'cb-hts-js-2026/cb-selected-clients' );
	$uses_marquee   = false;

	foreach ( $marquee_blocks as $block_name ) {
	...
	$rel = '/blocks/_shared/marquee-view.js';
	$abs = get_template_directory() . $rel;
	if ( file_exists( $abs ) ) {
		wp_enqueue_script( 'cb-hts-js-2026-marquee-view', get_template_directory_uri() . $rel, array( 'gsap' ), filemtime( $abs ), true );
	}
}
add_action( 'wp_enqueue_scripts', 'cb_hts_js_2026_enqueue_marquee_view_script' );
 ...
 * @return array
 */
function cb_hts_js_2026_core_block_type_args( $args, $name ) {
	$GLOBALS['cb_hts_js_2026_original_block_categories'][ $name ] = isset( $args['category'] ) ? $args['category'] : '';
	...
	if ( in_array( $name, $wrapped_blocks, true ) ) {
		$args['render_callback'] = 'cb_hts_js_2026_wrap_block_in_container';
	}

	return $args;
}
add_filter( 'register_block_type_args', 'cb_hts_js_2026_core_block_type_args', 10, 2 );
 ...
 * @return array
 */
function cb_hts_js_2026_register_gutenberg_block_category( $categories ) {
...
add_filter( 'block_categories_all', 'cb_hts_js_2026_register_gutenberg_block_category' );
 ...
 * cb_hts_js_2026_get_disallowed_gutenberg_blocks()), which silently hid
 * every block registered under a colliding "theme" slug.
 *
 * @param array $categories Existing block categories.
 * @return array
 */
function cb_hts_js_2026_register_theme_block_category( $categories ) {
	$theme = wp_get_theme();
...
add_filter( 'block_categories_all', 'cb_hts_js_2026_register_theme_block_category' );
 ...
 */
function cb_hts_js_2026_get_disallowed_gutenberg_blocks() {
	return array(
		'categories'        => array(
			'theme',
			'embed',
 ...
 * @return bool|array
 */
function cb_hts_js_2026_disallow_unwanted_core_blocks( $allowed_block_types ) {
	$registered_blocks       = WP_Block_Type_Registry::get_instance()->get_all_registered();
	$disallowed_gutenberg    = cb_hts_js_2026_get_disallowed_gutenberg_blocks();
	$disallowed_blocks = array_keys(
		array_filter(
			$registered_blocks,
			static function ( $block_type, $block_name ) use ( $disallowed_gutenberg ) {
				$original_categories    = isset( $GLOBALS['cb_hts_js_2026_original_block_categories'] ) ? $GLOBALS['cb_hts_js_2026_original_block_categories'] : array();
 ...
 * @return string
 */
function cb_hts_js_2026_wrap_block_in_container( $attributes, $content ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
