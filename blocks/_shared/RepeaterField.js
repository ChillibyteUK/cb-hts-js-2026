...
export default function RepeaterField( { label, value, onChange, fields, emptyRow, layout = 'row' } ) {
	...
	return (
		<div
			className={
				isColumn
					? 'cb-hts-js-2026-repeater-field cb-hts-js-2026-repeater-field--column'
					: 'cb-hts-js-2026-repeater-field'
			}
		>
			<label className="cb-hts-js-2026-editor-field__label">{ label }</label>
			{ ! isColumn && rows.length > 0 && (
				<div className="cb-hts-js-2026-repeater-field__header">
					<span className="cb-hts-js-2026-repeater-field__number-spacer" />
					{ fields.map( ( field ) => (
						<span
							key={ field.name }
							className={
								'image' === field.type || 'file' === field.type
									? 'cb-hts-js-2026-repeater-field__header-cell cb-hts-js-2026-repeater-field__header-cell--image'
									: 'cb-hts-js-2026-repeater-field__header-cell'
							}
						>
							{ field.label }
						</span>
			...
			{ rows.map( ( row, index ) => (
				<div className="cb-hts-js-2026-repeater-field__row" key={ index }>
					...
					{ fields.map( ( field ) => {
							...
							return (
								<MediaUploadCheck key={ field.name }>
									<MediaUpload
										...
										render={ ( { open } ) => (
											<div className="cb-hts-js-2026-repeater-field__image">
												{ row[ `${ field.name }Url` ] && (
						...
						if ( 'link' === field.type ) {
							return (
								<div className="cb-hts-js-2026-repeater-field__link" key={ field.name }>
									<TextControl
										label={ __( `${ field.label } Title`, 'cb-hts-js-2026' ) }
									...
									<TextControl
										type="url"
										label={ __( `${ field.label } URL`, 'cb-hts-js-2026' ) }
										hideLabelFromVision={ ! isColumn }
										value={ row[ field.name ] || '' }
									...
									{ field.linkTarget && (
										<ToggleControl
											label={ __( `Open ${ field.label } in a new tab`, 'cb-hts-js-2026' ) }
								...
								<MediaUploadCheck key={ field.name }>
									<MediaUpload
										...
										render={ ( { open } ) => (
											<div className="cb-hts-js-2026-repeater-field__image">
												{ row[ `${ field.name }Name` ] && (
													<span className="cb-hts-js-2026-repeater-field__file-name">
														{ row[ `${ field.name }Name` ] }
													</span>
												) }
												<Button variant="secondary" size="small" onClick={ open }>
					...
					<div className="cb-hts-js-2026-repeater-field__row-actions">
						<Button
							size="small"
							label={ __( 'Move up', 'cb-hts-js-2026' ) }
						...
						<Button
							size="small"
							label={ __( 'Move down', 'cb-hts-js-2026' ) }
							onClick={ () => moveRow( index, 1 ) }
