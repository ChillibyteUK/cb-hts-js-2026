...
while ( $products->have_posts() ) {
	...
	$cards[]  = array(
		...
		'tag'       => get_post_meta( $ppost_id, 'tag', true ),
		'intro'     => get_post_meta( $ppost_id, 'card_intro', true ),
	);
}
wp_reset_postdata();
...
$block_id         = $attributes['anchor'] ?? wp_unique_id( 'cb-products-nav-' );
$background_color = $attributes['backgroundColor'] ?? '';
$line_class       = 'dark-lines';

if ( $background_color ) {
	if ( preg_match( '/(\d+)(?!.*\d)/', $background_color, $matches ) ) {
