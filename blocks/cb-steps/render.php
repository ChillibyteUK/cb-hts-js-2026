 ...
 *
 * Step number/icon/title are fixed; only each step's body copy is editable
 * (see src/edit.js for why this isn't the generic RepeaterField pattern).
 *
...
$wrapper_attributes = get_block_wrapper_attributes( array( 'class' => 'process' ) );
?>
<section <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- get_block_wrapper_attributes() already escapes. ?>>
	<div class="container">
		<div class="process-header">
			...
			if ( $headline ) {
				?>
				<h2 class="process-headline h2"><?php echo wp_kses( $headline, $headline_allowed ); ?></h2>
				<?php
			}
			if ( $intro ) {
				?>
				<p class="process-sub prose-md"><?php echo nl2br( esc_html( $intro ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- nl2br() output of an already-escaped string. ?></p>
				<?php
			}
			?>
		</div>
		...
		<div class="process-steps">
			...
			foreach ( $step_defaults as $index => $step ) {
				...
				<div class="process-step">
					...
					<div class="step-icon"><?php echo $step['icon']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- literal icon markup defined above, not user input. ?></div>
					<div class="step-title"><?php echo wp_kses_post( $step['title'] ); ?></div>
					<div class="step-body"><?php echo wp_kses( $body, $br_allowed ); ?></div>
				</div>
				<?php
			}
			?>
